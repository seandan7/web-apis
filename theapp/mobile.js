      ...
      .then(permissionState => {
        if (permissionState === 'granted') {
          window.addEventListener('deviceorientation', (event) => {
            mobileTextDiv.innerHTML = "Roration at: " + Math.floor(event.alpha) + "deg"
            document.getElementById('emoji').style.transform = "rotate(-"+event.alpha+"deg)"
      ...
      .then(permissionState => {
        if (permissionState === 'granted') {
          ...
          window.addEventListener('devicemotion', (e) => {
            ...
            motionTextDiv.innerHTML = "Spinning at " + Math.floor(Math.abs(e.rotationRate.alpha)) + " Per Second"
          });
          setInterval(() => {
            maxSpinDiv.innerHTML = "Your max spin for the last 5 seconds: "+ Math.floor(Math.abs(maxSpinRate));
            maxSpinRate = 0;
          }, 5000)
        }
      })
